...
const get = async (view, options) => {
  if (view === 'poster' || view === 'card' || view === 'card-overlay') {
    // tmdb
    ...
    if (!filterImages.length) return null;
    ...
    for (const key in filterImages) {
      ...
      if (element.iso_639_1 === 'en' && !imageEng) imageEng = element.file_path;
    }
    return await to_base64(tmdbBaseUrl + (imageEng || filterImages[0].file_path));
  } else if (view === 'fanart' || view === 'fanart-overlay' || view === 'banner') {
    // fanart
    ...
    try {
      ...
      const filterImages =
        images[
          view === 'fanart' || view === 'fanart-overlay'
      ...
      for (const key in filterImages) {
        const element = filterImages[key];
        if (element.lang === options.language) return await to_base64(element.url);
        if (element.lang === 'en' && !imageEng) imageEng = element.url;
      }

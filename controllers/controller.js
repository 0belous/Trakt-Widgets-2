...
const profile = async (req, next, view, language) => {
  ...
  return await trakt.users
    ...
    .then(async response => {
      ...
      if (response.images?.avatar?.full)
      ...
      return await Promise.allSettled([
        ...
        } else if (stats.status !== 'fulfilled') return next(createError(400, stats.reason));
        else if (history.status !== 'fulfilled') return next(createError(400, history.reason));
      });
    })
    .catch(next);
};
...
const watched = async (req, next, view, language) => {
  ...
  if (view === 'fanart-overlay' || view === 'card-overlay') {
    profile_image = await trakt.users
      .profile({
        username: slug,
        extended: 'full',
      })
  ...
  return await trakt.users
    ...
    .catch(err => {
      if (err.message.includes('401')) return next(createError(401, 'Account private'));
      return next(err);
    });
};
